...
if ($version === 1) {
    ...
    require $currentDirectory . 'api' . DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR . 'autoload.php';
}
...
$status = $counter . ' ' . $mot;
if ($version === 1) {
    $apiResponse = updateStatus($status, array('settings' => 'countingSettings.php'));
} elseif ($version === 2) {
    $config = require
        $currentDirectory
        . '..'
    ...
    $countingBot3Config = $checkConfig($twitterApiConfig, 'countingbot3');

    $consumerKey = $checkConfig($countingBot3Config, 'consumer_key');
    $consumerSecret = $checkConfig($countingBot3Config, 'consumer_secret');
    $accessToken = $checkConfig($countingBot3Config, 'oauth_access_token');
    $accessTokenSecret = $checkConfig($countingBot3Config, 'oauth_access_token_secret');

    $twitter = new TwitterPoster($accessToken, $accessTokenSecret, $consumerKey, $consumerSecret);

    $apiResponse = $twitter->updateStatus($status, TwitterPoster::VERSION_2);
}

echo '<br>Tweet à poster : ' . $status;
